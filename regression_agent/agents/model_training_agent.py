...
class SFNModelTrainingAgent(SFNAgent):
    ...
    def execute_task(self, task: Task) -> Dict:
        """
        Train a specific regression model and return metrics and model object
        """
        ...
        for attempt in range(self.max_retries):
            try:
                if last_error:
                    print(
                        f">>>>>> Retry attempt {attempt + 1}/{self.max_retries} for model training. Previous error: {str(last_error)}")
                ...
                if target_column is None or target_column not in train_df.columns:
                    print(
                        f"Warning: Target column '{target_column}' not found. Using all records.")
                ...
                if len(train_df_filtered) == 0 or len(valid_df_filtered) == 0:
                    print(
                        "Warning: No valid records found after filtering. Using original data.")
                    train_df_filtered = train_df
                    valid_df_filtered = valid_df
                ...
                print(
                    f">>>>>> Successfully trained model on attempt {attempt + 1}")

                print(globals_dict.get('metrics', {}))
                return {
                ...
                if attempt == self.max_retries - 1:
                    print(
                        f">>>>>> All {self.max_retries} attempts failed. Returning error response.")
                    return {
                        'metrics': {
    ...
    def _get_data_info(self, data: Dict) -> Dict:
        ...
        try:
            ...
            train_mask = pd.notna(train_df[target_col]) & (
                train_df[target_col] != '') & (train_df[target_col] != 'None')
            valid_mask = pd.notna(valid_df[target_col]) & (
                valid_df[target_col] != '') & (valid_df[target_col] != 'None')
            ...
            try:
                dtypes_dict = {str(k): str(v)
                               for k, v in train_df.dtypes.to_dict().items()}
            except:
                # Fallback for dtypes
