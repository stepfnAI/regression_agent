    ...
    def _handle_leakage_detection(self):
        ...
        try:
            ...
            if severe_features or review_features:
                ...
                available_features = [
                    col for col in df.columns if col not in mapped_columns]
                default_selected = [f for f in (
                    severe_features + review_features) if f not in mapped_columns]
                ...
                if self.view.display_button("Confirm Feature Removal"):
                    if features_to_remove:
                        ...
                        self.session.set('removed_features',
                                         features_to_remove)
                ...
                self.view.show_message(
                    "✅ No target leakage detected", "success")
                return True

        except Exception as e:
            self.view.show_message(
                f"Error in leakage detection: {str(e)}", "error")
            return False
    ...
    def _handle_categorical_features(self):
        ...
        try:
            ...
            with self.view.display_spinner('🤖 AI is analyzing categorical features...'):
                ...
                result = self.categorical_agent.execute_task(
                    Task("Analyze categorical features", data=task_data))
            ...
            self.view.show_message(
                f"Error in categorical feature processing: {str(e)}", "error")
            return False
